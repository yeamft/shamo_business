import { T as TSS_SERVER_FUNCTION, c as createServerFn } from "./server-DqHXt5kP.mjs";
import { m as mergeCategories, s as slugifyCategoryName } from "./categories-i6a11iUi.mjs";
...
function mapVideoRow(post2) {
  return {
    id: post2.id,
    titleEn: post2.title_en,
    titleAm: post2.title_am,
    thumb: post2.thumb || getYouTubeThumbnail(post2.youtube_url),
    youtubeUrl: normalizeYouTubeEmbedUrl(post2.youtube_url),
    duration: post2.duration,
    views: post2.views,
    channel: post2.channel,
    channelAm: post2.channel_am,
    category: post2.category,
    postedDays: post2.posted_days,
    status: post2.status,
    createdAtLabel: post2.created_at_label,
    keywords: post2.keywords,
    descriptionEn: post2.description_en,
    descriptionAm: post2.description_am,
    shareTo: post2.share_to,
    source: post2.source
  };
}
...
function isMissingAdminCategoriesTableError(error) {
...
const getAdminSnapshot = createServerFn({
  ...
  const [{
    ...
    data: settingsRow
  }, categoriesResult] = await Promise.all([supabase.from("admin_videos").select("*").order("inserted_at", {
    ascending: false
  }), supabase.from("job_registrations").select("*").order("created_at", {
    ascending: false
  }), supabase.from("admin_settings").select("*").eq("id", 1).single(), supabase.from("admin_categories").select("*").order("created_at", {
    ascending: true
  })]);
  const mappedPosts = posts?.map(mapVideoRow) ?? [];
  const mappedCategories = isMissingAdminCategoriesTableError(categoriesResult.error) ? [] : categoriesResult.data?.map(mapCategoryRow) ?? [];
  return {
    posts: mappedPosts,
...
const getPublicCategories_createServerFn_handler = createServerRpc({
  id: "ec2e0477309b7f8ed212d59087cc93615ab9c0e88572471df7091c81f7226ded",
  name: "getPublicCategories",
  filename: "src/lib/api/admin.functions.ts"
}, (opts) => getPublicCategories.__executeServer(opts));
const getPublicCategories = createServerFn({
  method: "GET"
}).handler(getPublicCategories_createServerFn_handler, async () => {
  const supabase = getSupabaseServerClient();
  const [{
    data: videos
  }, categoriesResult] = await Promise.all([supabase.from("admin_videos").select("category").eq("status", "Published"), supabase.from("admin_categories").select("*").order("created_at", {
    ascending: true
  })]);
  if (isMissingAdminCategoriesTableError(categoriesResult.error)) {
    return mergeCategories([], videos ?? []);
  }
  return mergeCategories(categoriesResult.data?.map(mapCategoryRow) ?? [], videos ?? []);
});
...
const createAdminCategory_createServerFn_handler = createServerRpc({
  id: "c770afb719c4ad8369a5674d4265fe8518da4ca5a2a6b95625397a7386f299e8",
  name: "createAdminCategory",
  filename: "src/lib/api/admin.functions.ts"
}, (opts) => createAdminCategory.__executeServer(opts));
const createAdminCategory = createServerFn({
  method: "POST"
}).validator(categorySchema).handler(createAdminCategory_createServerFn_handler, async ({
  data
}) => {
  const supabase = getSupabaseServerClient();
  const {
    data: category,
    error
  } = await supabase.from("admin_categories").upsert({
    id: data.id,
...
const updateAdminCategory_createServerFn_handler = createServerRpc({
  id: "88879d3eb383c60adf58f5afd95d6ecf2a70f763665cc0b834464d0865a3eda7",
  name: "updateAdminCategory",
  filename: "src/lib/api/admin.functions.ts"
}, (opts) => updateAdminCategory.__executeServer(opts));
const updateAdminCategory = createServerFn({
  method: "POST"
}).validator(categorySchema.extend({
  categoryId: stringType().trim().min(1)
})).handler(updateAdminCategory_createServerFn_handler, async ({
  data
}) => {
  const supabase = getSupabaseServerClient();
...
const deleteAdminCategory_createServerFn_handler = createServerRpc({
  id: "fb508a905031f10eb743540d152ea23d9d284054cf764ad51ee8f686603d5efe",
...
const deleteAdminCategory = createServerFn({
  ...
  categoryId: stringType().trim().min(1)
})).handler(deleteAdminCategory_createServerFn_handler, async ({
  data
}) => {
  const supabase = getSupabaseServerClient();
  const {
    count
  } = await supabase.from("admin_videos").select("id", {
    count: "exact",
    head: true
  }).eq("category", data.categoryId);
  if ((count ?? 0) > 0) {
...
export {
  ...
  createAdminCategory_createServerFn_handler,
  createAdminPost_createServerFn_handler,
  createVideoComment_createServerFn_handler,
  deleteAdminCategory_createServerFn_handler,
  ...
  getPublicCategories_createServerFn_handler,
  ...
  updateAdminCategory_createServerFn_handler,
